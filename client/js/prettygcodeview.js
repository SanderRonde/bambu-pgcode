$(function () {
    ...
    $("#pgc2dcanvas").on("wheel", function (e) {
        ...
        if (camera2d.zoom < 0.1) camera2d.zoom = 0.1;
        camera2d.updateProjectionMatrix();
        needRender = true;
    ...
    $("#play-button").on("click", function (e) {
        ...
        forceNoSync = false;
        if (!simPlaying) $("#play-button").html("&#x25B6;");
        else $("#play-button").html("&#x23F8;");
    });
    $("#faster-button").on("click", function (e) {
        playbackRate = playbackRate * 2;
        if (playbackRate > 64) playbackRate = 64;
    });
    $("#slower-button").on("click", function (e) {
        playbackRate = playbackRate / 2;
        if (playbackRate < 0.125) playbackRate = 0.125;
    });
    ...
    $("#pgccanvas").on("dragover", function (e) {
        e.preventDefault();
        e.stopPropagation();
    });
    $("#pgccanvas").on("dragenter", function (e) {
        e.preventDefault();
        e.stopPropagation();
    });
    $("#pgccanvas").on("drop", function (e) {
    ...
    function initGui() {
        ...
        if (true) {
            ...
            gui.add(pgSettings, "lightTheme").onFinishChange(function () {
                ...
                else myScene.background = null; //new THREE.Color(0xd0d0d0);
            });
            ...
            if (true) {
                ...
                if (pgcServer == null) {
                    pgcServer =
                        ...
                        ":" +
                        defaultMoonrakerPort;
                    console.log(
                        "No server configured. Trying default:" + pgcServer
                    );
                    if (pgcServer.startsWith("file")) {
                        pgcServer =
                            "http://fluiddpi.local:" + defaultMoonrakerPort;
                        console.log(
                            "Running from file. Setting file_url:" + pgcServer
                        );
                    }
                } else {
    ...
    function initThree() {
        renderer = new THREE.WebGLRenderer({
            canvas: document.getElementById("pgccanvas"),
            antialias: pgSettings.antialias,
            alpha: true,
        });
        //todo. is this right?
        ...
        else camera2d.position.set(0, 0, 500);
        ...
        if (pgSettings.saveCamera && localStorage.getItem("pgcCameraPos")) {
            ...
            try {
                ...
                if (camera2d.zoom) camera2d.zoom = camPos.zoom;
                camera2d.updateProjectionMatrix();
            } catch {}
        }
        ...
        if (pgSettings.lightTheme) scene.background = new THREE.Color(0xd0d0d0);
        else scene.background = null; //new THREE.Color(0xd0d0d0);
        ...
        if (pgSettings.fatLines) {
            ...
            highlightMaterial.resolution.set(
                window.innerWidth,
                window.innerHeight
            );
        } else {
        ...
        function animate() {
            ...
            if (printHeadSim && simPlaying) {
                ...
                if (curState.filePos) curSimFilePos = curState.filePos;

                //console.log(fpDelta)
            }
            if (
                curPrinterState &&
                (curPrinterState.startsWith("printing") ||
                    curPrinterState == "paused") &&
                pgSettings.syncToProgress &&
                !forceNoSync
            ) {
                //if(!forceNoSync || )
                if (nozzleModel && printHeadSim) {
                    ...
                    if (extrudingLineGroup && curState.startPoint) {
                        if (!curState.extrude)
                            ...
                            var vectToCurEnd = curState.position
                                .clone()
                                .sub(curState.startPoint);
                            ...
                            extrudingLineGroup.position.copy(
                                curState.startPoint
                            );
            ...
            if (updated) {
                ...
                if (pgSettings.saveCamera) {
                    let camStr = JSON.stringify({
                        pos: cameraControls.getPosition(),
            ...
            if (resizeCanvasToDisplaySize()) needRender = true;
            ...
            if (needRender) {
                ...
                if (pgSettings.show2d) {
                    ...
                    if (nozzleModel) nozzleModel.visible = false;
    ...
    function resetCamera() {
        ...
        else cameraControls.setTarget(0, 0, 0, false);
    }
    ...
    function updateJob(job, apiKey) {
        if (job instanceof File) {
            if (viewInitialized) {
                ...
                if (currentLayerCopy) myScene.remove(currentLayerCopy);
